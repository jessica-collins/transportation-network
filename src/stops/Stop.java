...
public class Stop implements Writeable {
    ...
    // the intermediate stops passengers at this stop will be travelling to next
    private Map<Stop, Passenger> nextStopPassengers;
    ...
    public Stop(String name, int x, int y) {
        ...
        this.nextStopPassengers = new HashMap<>();
    }
     ...
     * (RoutingTable.addNeighbour(Stop)) in the routing table for use when routing
     * passengers.
     *
     ...
     *
     * <p>If the passenger's destination is null, it should be added to the
     * stop, but the stop does not need to determine where to route it.
     *
     * <p>When a passenger arrives at a stop, the stop should use its routing
     * table to determine which neighbouring stop to route the passenger to based
     * on the passenger's desired final destination see
    ...
    public void addPassenger(Passenger passenger) {
        if (passenger == null) {
            return;
        } else if (passenger.getDestination() == null) {
            this.passengers.add(passenger);
        } else {
            this.passengers.add(passenger);
            this.nextStopPassengers.put(this.routingTable.nextStop
                    (passenger.getDestination()), passenger);
        }
    }
     ...
     *
     * <p>Otherwise, this method should update the vehicle's location to be the
    ...
    public void transportDepart(PublicTransport transport, Stop nextStop) {
        ...
        for (Passenger passenger : this.passengers) {
            if (this.nextStopPassengers.containsKey
